  ...
  return (
    <Card sx={{ height: "100%" }}>
      ...
      <CardContent sx={{ padding: 2 }}>
        ...
        >
          <StarIcon sx={{ marginRight: 1 }} /> Nombre Evaluations:{" "}
          {_count.evaluations}
        </Typography>
      </CardContent>
  ...
  const loadAllOfferors = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/offreurs/search?fname=${searchTerm}`,
        {
          headers: {
    ...
    <>
      ...
      >
        ...
        <div style={{ padding: "20px" }}>
          ...
          >
            {" "}
            {(filteredOfferors.length > 0 ? filteredOfferors : allOfferors).map(
              (offreur) => (
